...
Usage:
    python -u merge_shards_hdf5.py \
        --shard_dir embeddings/shards_platon_mimic \
        --npz_output Echo_JEPA_Embeddings/768d/jepa_clips_platon_mimic.npz \
        --hdf5_output_dir Echo_JEPA_Embeddings/768d
"""
...
def main():
    ...
    for emb, sid, vid in zip(embeddings, study_ids, video_ids):
        fname = Path(str(vid)).name  # handle full paths or bare filenames
